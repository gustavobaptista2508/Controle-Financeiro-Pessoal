...
public final class DocumentParser {
    ...
    private static final Pattern MONEY_DOT = Pattern.compile("(?<!\\d)([-+]?\\s*(?:R\\$\\s*)?\\d+\\.\\d{2})(?!\\d)", Pattern.CASE_INSENSITIVE);
    ...
    public static JSONObject parseStatement(String raw, String fileName) throws Exception {
        ...
        if (normalized.contains("<stmttrn>") || normalized.contains("<ofx>")) {
            return parseOfx(text, fileName);
        }
        String first = firstNonEmptyLine(text);
        if (looksLikeCsvHeader(first)) {
            JSONObject csv = parseCsv(text, fileName);
            if (csv.optJSONArray("rows") != null && csv.optJSONArray("rows").length() > 0) return csv;
        }
    ...
    private static JSONObject parseOfx(String text, String fileName) throws Exception {
        ...
        Pattern blockPattern = Pattern.compile("(?is)<STMTTRN>(.*?)(?=<STMTTRN>|</BANKTRANLIST>|</OFX>|$)");
        Matcher blocks = blockPattern.matcher(text);
        while (blocks.find() && rows.length() < 800) {
            String b = blocks.group(1);
            String amountRaw = ofxTag(b, "TRNAMT");
            BigDecimal signed = parseFlexibleMoney(amountRaw);
            if (signed == null || signed.compareTo(BigDecimal.ZERO) == 0) continue;
            String date = normalizeOfxDate(ofxTag(b, "DTPOSTED"));
            if (date.isEmpty()) continue;
            String name = firstNonEmpty(ofxTag(b, "NAME"), ofxTag(b, "MEMO"), ofxTag(b, "CHECKNUM"), "Lançamento bancário");
            String memo = ofxTag(b, "MEMO");
            String fitid = ofxTag(b, "FITID");
            String type = signed.signum() < 0 ? "expense" : "income";
            BigDecimal amount = signed.abs().setScale(2, RoundingMode.HALF_UP);
            String key = date + "|" + fold(name) + "|" + type + "|" + amount.toPlainString();
            if (!seen.add(key)) continue;
            JSONObject row = new JSONObject();
            row.put("date", date); row.put("description", tidyDescription(name)); row.put("type", type);
    ...
    private static JSONObject parseCsv(String text, String fileName) throws Exception {
        ...
        for (int i = 0; i < Math.min(lines.length, 20); i++) {
            ...
            char d = detectDelimiter(line);
            List<String> cells = splitCsv(line, d);
            String joined = fold(join(cells, " "));
        ...
        for (int i = headerIndex + 1; i < lines.length && rows.length() < 800; i++) {
            ...
            String date = normalizeDate(cell(cells, dateCol));
            String desc = tidyDescription(cell(cells, descCol));
            if (date.isEmpty() || desc.isEmpty() || isBalanceLine(desc)) continue;
            BigDecimal signed = null;
            String type = null;
            if (debitCol >= 0 || creditCol >= 0) {
                BigDecimal debit = parseFlexibleMoney(cell(cells, debitCol));
                BigDecimal credit = parseFlexibleMoney(cell(cells, creditCol));
                if (debit != null && debit.abs().compareTo(BigDecimal.ZERO) > 0) {
                    signed = debit.abs(); type = "expense";
                } else if (credit != null && credit.abs().compareTo(BigDecimal.ZERO) > 0) {
                    signed = credit.abs(); type = "income";
                }
            }
            if (signed == null && valueCol >= 0) {
                BigDecimal v = parseFlexibleMoney(cell(cells, valueCol));
                if (v != null && v.compareTo(BigDecimal.ZERO) != 0) {
                    type = v.signum() < 0 ? "expense" : "income";
                    signed = v.abs();
                }
            }
            if (signed == null || type == null) continue;
            BigDecimal amount = signed.abs().setScale(2, RoundingMode.HALF_UP);
            String key = date + "|" + fold(desc) + "|" + type + "|" + amount.toPlainString();
            if (!seen.add(key)) continue;
            JSONObject row = new JSONObject();
            row.put("date", date); row.put("description", desc); row.put("type", type); row.put("amount", amount.doubleValue());
            String external = cell(cells, idCol).trim(); if (!external.isEmpty()) row.put("external_id", external);
            rows.put(row);
        }
    ...
    private static JSONObject parseGenericStatement(String text, String fileName) throws Exception {
        ...
        int inferredYear = now.get(Calendar.YEAR);
        Pattern linePattern = Pattern.compile("^\\s*(\\d{2}[/-]\\d{2}(?:[/-]\\d{2,4})?)\\s+(.+?)\\s+([-+]?\\s*(?:R\\$\\s*)?(?:\\d{1,3}(?:\\.\\d{3})+|\\d+),\\d{2})(?:\\s*([DC]))?\\s*$", Pattern.CASE_INSENSITIVE);
        Pattern dotPattern = Pattern.compile("^\\s*(\\d{2}[/-]\\d{2}(?:[/-]\\d{2,4})?)\\s+(.+?)\\s+([-+]?\\s*(?:R\\$\\s*)?\\d+\\.\\d{2})(?:\\s*([DC]))?\\s*$", Pattern.CASE_INSENSITIVE);
        for (String line : lines) {
            if (rows.length() >= 800) break;
            String s = line.trim(); if (s.length() < 8) continue;
            Matcher m = linePattern.matcher(s); if (!m.matches()) { m = dotPattern.matcher(s); if (!m.matches()) continue; }
            String date = normalizeDateWithYear(m.group(1), inferredYear);
            String desc = tidyDescription(m.group(2));
            if (date.isEmpty() || desc.isEmpty() || isBalanceLine(desc)) continue;
            BigDecimal signed = parseFlexibleMoney(m.group(3)); if (signed == null || signed.compareTo(BigDecimal.ZERO) == 0) continue;
            String dc = m.group(4);
            String type;
            if (dc != null && dc.equalsIgnoreCase("D")) type = "expense";
            else if (dc != null && dc.equalsIgnoreCase("C")) type = "income";
            else type = signed.signum() < 0 ? "expense" : "income";
            BigDecimal amount = signed.abs().setScale(2, RoundingMode.HALF_UP);
            String key = date + "|" + fold(desc) + "|" + type + "|" + amount.toPlainString();
            if (!seen.add(key)) continue;
    ...
    private static JSONObject statementResult(JSONArray rows, String fileName, String format) throws Exception {
        ...
        out.put("file_name", fileName == null ? "extrato" : fileName); out.put("rows", rows); out.put("count", rows.length());
        return out;
    }

    private static BigDecimal guessReceiptTotal(String[] lines) {
        BigDecimal best = null; int bestScore = -999;
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i]; String f = fold(line);
            Matcher m = MONEY_BR.matcher(line);
            while (m.find()) {
                ...
                if (v.compareTo(new BigDecimal("0.50")) < 0) score -= 50;
    ...
    private static String guessMerchant(String[] lines) {
        for (int i = 0; i < Math.min(lines.length, 12); i++) {
            String s = lines[i].trim(); String f = fold(s);
            ...
            if (MONEY_BR.matcher(s).find() || DATE_DMY.matcher(s).find()) continue;
            if (s.matches(".*\\d{8,}.*")) continue;
    ...
    private static String guessPayment(String text) {
        ...
        if (f.contains("credito") || f.contains("credit")) return "Cartão de crédito";
        if (f.contains("debito") || f.contains("debit")) return "Cartão de débito";
        if (f.contains("dinheiro")) return "Dinheiro";
        if (f.contains("voucher")) return "Voucher";
        return "Não identificado";
    }

    private static String ofxTag(String block, String tag) {
        Pattern p = Pattern.compile("(?is)<" + Pattern.quote(tag) + ">\\s*([^<\\r\\n]+)");
        Matcher m = p.matcher(block); return m.find() ? m.group(1).trim() : "";
    }

    private static String normalizeOfxDate(String value) {
        if (value == null) return "";
        Matcher m = Pattern.compile("(\\d{4})(\\d{2})(\\d{2})").matcher(value);
        return m.find() ? m.group(1) + "-" + m.group(2) + "-" + m.group(3) : "";
    }

    private static boolean looksLikeCsvHeader(String line) {
        String f = fold(line);
        return (line.contains(";") || line.contains(",") || line.contains("\\t")) &&
            (f.contains("data") || f.contains("date")) &&
            (f.contains("valor") || f.contains("amount") || f.contains("debito") || f.contains("credito"));
    }

    private static char detectDelimiter(String line) {
        int semi = count(line, ';'), comma = count(line, ','), tab = count(line, '\t');
    ...
    private static List<String> splitCsv(String line, char delimiter) {
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            } else if (ch == delimiter && !quoted) { out.add(cur.toString().trim()); cur.setLength(0); }
            else cur.append(ch);
        }
    ...
    private static int findColumn(List<String> header, String... names) {
        for (int i = 0; i < header.size(); i++) {
            String h = fold(header.get(i));
            for (String n : names) if (h.equals(n) || h.contains(n)) return i;
        }
        return -1;
    }
    ...
    public static BigDecimal parseFlexibleMoney(String raw) {
        ...
        String s = raw.trim().replace("R$", "").replace(" ", ""); if (s.isEmpty()) return null;
        ...
        try {
            if (s.contains(",")) s = s.replace(".", "").replace(',', '.');
                ...
                else if (dots == 1 && s.matches("[-+]?\\d{1,3}\\.\\d{3}")) s = s.replace(".", "");
            }
            BigDecimal v = new BigDecimal(s); return paren ? v.negate() : v;
    ...
    private static String normalizeDateWithYear(String raw, int inferredYear) {
        if (raw == null) return ""; String s = raw.trim();
        Matcher ymd = DATE_YMD.matcher(s); if (ymd.find()) return ymd.group(1) + "-" + ymd.group(2) + "-" + ymd.group(3);
        Matcher dmy = DATE_DMY.matcher(s); if (dmy.find()) return toIso(dmy.group(1), dmy.group(2), dmy.group(3));
        Matcher dm = DATE_DM.matcher(s); if (dm.find()) return String.format(Locale.US, "%04d-%02d-%02d", inferredYear, Integer.parseInt(dm.group(2)), Integer.parseInt(dm.group(1)));
        return "";
    }

    private static String toIso(String dd, String mm, String yy) {
        int y = Integer.parseInt(yy); if (y < 100) y += y >= 70 ? 1900 : 2000;
        int m = Integer.parseInt(mm), d = Integer.parseInt(dd);
        if (m < 1 || m > 12 || d < 1 || d > 31) return "";
        return String.format(Locale.US, "%04d-%02d-%02d", y, m, d);
    }

    private static boolean isBalanceLine(String desc) {
        String f = fold(desc); return f.contains("saldo anterior") || f.equals("saldo") || f.startsWith("saldo ") || f.contains("saldo do dia") || f.contains("saldo disponivel") || f.contains("total do dia");
    }
