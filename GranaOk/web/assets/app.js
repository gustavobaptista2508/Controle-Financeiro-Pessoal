...
async function api(action,data={}){
  const r=await fetch('api.php?action='+encodeURIComponent(action),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data),credentials:'same-origin'});
  let d={};try{d=await r.json()}catch{d={ok:false,error:'Resposta inválida do servidor.'}}
  if(r.status===401){showLogin();throw new Error(d.error||'Sessão expirada.')}
...
function setTitle(t,s='GranaOk Web'){ $('#page-title').textContent=t;$('#page-subtitle').textContent=s; }
...
$('#login-form').onsubmit=async e=>{e.preventDefault();const out=$('#login-out');out.innerHTML=note('Entrando...');try{await api('login',{password:$('#login-password').value});out.innerHTML='';showApp();dashboard()}catch(err){out.innerHTML=note(err.message,'err')}};

(async()=>{try{const s=await api('session');if(s.authenticated){showApp();dashboard()}else showLogin()}catch{showLogin()}})();
})();