...
async function invoice(cardId,m){
  ...
  try{
    ...
    '<div class="card"><small class="muted">Total da fatura</small><div class="invoice-total">'+money(d.total)+'</div><p>'+badge(d.status)+'</p>'+(d.status==='paid'
      ? '<p>Pago em <b>'+br(d.paid_date)+'</b></p>'+(Number(d.balance_applied)===1
          ? '<div class="note ok">Pagamento já descontado do saldo'+(d.paid_account_name?' da conta <b>'+esc(d.paid_account_name)+'</b>':'')+'.</div>'
          : '<div class="note">Esta fatura já está paga, mas o pagamento ainda não foi aplicado ao saldo de uma conta.</div><label>Conta usada no pagamento</label><select id="iaccount"><option value="">Selecione a conta</option>'+ctx.accounts.filter(a=>Number(a.active)!==0).map(a=>'<option value="'+a.id+'">'+esc(a.name)+' · '+money(a.current_balance)+'</option>').join('')+'</select><button class="primary" id="iapply" style="margin-top:10px">Aplicar pagamento ao saldo</button>')
        +'<button class="secondary" id="ir" style="margin-top:10px">Reabrir fatura</button>'
    ...
    if($('#iapply'))$('#iapply').onclick=async()=>{const accountId=Number($('#iaccount')?.value||0);if(!accountId)return alert('Selecione a conta usada no pagamento.');try{await api('invoice_pay',{card_id:cardId,month:m,paid_date:d.paid_date||today(),account_id:accountId});invoice(cardId,m)}catch(e){alert(e.message)}};
