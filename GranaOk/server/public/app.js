...
async function more(){
 ...
 c.innerHTML='<div class="heading"><div><h1>Mais</h1><div class="muted">'+esc(user?.display_name||user?.username||'')+'</div></div></div>'+
 '<div class="card"><h3>Contas</h3>'+ctx.accounts.map(a=>'<div class="row"><div class="main"><b>'+esc(a.name)+'</b><small>'+esc(a.bank_code||'')+'</small></div><div class="amount">'+money(a.current_balance)+'</div></div>').join('')+'</div>'+
 (user?.role==='admin'?'<div class="card" style="margin-top:12px"><div class="heading"><div><h3>Usuários</h3><div class="muted">Acesso ao GranaOk</div></div><button class="primary" id="unew">＋</button></div><div id="users-list"><div class="empty">Carregando...</div></div></div>':'')+
 '<div class="card" style="margin-top:12px"><p class="muted">GranaOk Web/PWA conectado pela API. No iPhone, use Compartilhar → Adicionar à Tela de Início.</p></div>';
 if(user?.role==='admin'){ $('#unew').onclick=userForm; loadUsers(); }
}
...
function userForm(){
 $('#modal-content').innerHTML='<h2>Novo usuário</h2><label>Nome exibido</label><input id="ud"><label>Usuário</label><input id="uu"><label>Senha</label><input id="up" type="password"><label>Perfil</label><select id="ur"><option value="user">Usuário</option><option value="readonly">Somente leitura</option><option value="admin">Administrador</option></select><label>Vincular a pessoa (opcional)</label><select id="uper"><option value="">—</option>'+ctx.people.map(p=>'<option value="'+p.id+'">'+esc(p.name)+'</option>').join('')+'</select><button class="primary" id="us" style="margin-top:12px">Criar</button><div id="uo"></div>';$('#modal').classList.remove('hidden');
 $('#us').onclick=async()=>{try{await usersApi('/api/users','POST',{display_name:$('#ud').value,username:$('#uu').value,password:$('#up').value,role:$('#ur').value,person_id:Number($('#uper').value||0)});$('#modal').classList.add('hidden');loadUsers()}catch(e){$('#uo').innerHTML=note(e.message,'err')}};
}
function passwordForm(id){
 $('#modal-content').innerHTML='<h2>Alterar senha</h2><label>Nova senha</label><input id="np" type="password"><button class="primary" id="nps" style="margin-top:12px">Salvar</button><div id="npo"></div>';$('#modal').classList.remove('hidden');
 $('#nps').onclick=async()=>{try{await usersApi('/api/users/'+id+'/password','POST',{password:$('#np').value});$('#modal').classList.add('hidden')}catch(e){$('#npo').innerHTML=note(e.message,'err')}};
}
